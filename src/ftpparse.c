...
static long guessyear(long month,long mday)
{
	long year;
	long t;
	...
	switch(*buf) {
				...
				if (buf[j] == ',') {
					switch(buf[i]) {
						...
						case 'm':
							...
							fp->mtime = ftp_i_date_from_unix_timestamp(getlong(buf + i + 1,j - i - 1));
							fp->mtime_given = ftp_btrue;
							break;
						case 'i':
		...
		case '-':
			...
			if (*buf == 'l') fp->flagtrycwd = fp->flagtryretr = 1;
					...
					//check for unix-style permissions and store them:
					if (*(buf+10) == ' ') {
							ftp_i_memcpy(fp->unix_permissions, buf, 0, 10);
					} else {
							fp->unix_permissions[0] = 0;
					}
					...
					switch(state) {
						...
						case 7: /* have size, month, mday */
							if ((j - i == 4) && (buf[i + 1] == ':')) {
								...
								return 0;
							fp->mtime = ftp_i_date_from_values(year, month, mday, hour, minute, 0);
							fp->mtime_given = ftp_btrue;
							fp->name = buf + j + 1;
							fp->namelen = len - j - 1;
							state = 8;
